...
void x_handle_error(struct x_connection *c, xcb_generic_error_t *ev) {
  ...
  if (c->pending_reply_head && c->pending_reply_head->sequence == ev->full_sequence) {
    if (c->pending_reply_head->action != PENDING_REPLY_ACTION_IGNORE) {
      x_log_error(LOG_LEVEL_ERROR, ev->full_sequence, ev->major_code, ev->minor_code,
                  ev->error_code);
    }
  ...
  x_log_error(LOG_LEVEL_WARN, ev->full_sequence, ev->major_code, ev->minor_code, ev->error_code);
}
...
winprop_t x_get_prop_with_offset(const struct x_connection *c, xcb_window_t w, xcb_atom_t atom,
  ...
  xcb_get_property_reply_t *r = xcb_get_property_reply(
      c->c,
      xcb_get_property(c->c, 0, w, atom, rtype, to_u32_checked(offset), to_u32_checked(length)),
      NULL);
  ...
  free(r);
  return (winprop_t){.ptr = NULL, .nitems = 0, .type = XCB_GET_PROPERTY_TYPE_ANY, .format = 0};
}
...
winprop_info_t x_get_prop_info(const struct x_connection *c, xcb_window_t w, xcb_atom_t atom) {
  ...
  if (!r) {
    ...
    free(e);
    return (winprop_info_t){.type = XCB_GET_PROPERTY_TYPE_ANY, .format = 0, .length = 0};
  }

  winprop_info_t winprop_info = {.type = r->type, .format = r->format, .length = r->bytes_after};
  free(r);
...
bool wid_get_text_prop(session_t *ps, xcb_window_t wid, xcb_atom_t prop, char ***pstrlst,
  ...
  if (type != XCB_ATOM_STRING && type != ps->atoms->aUTF8_STRING &&
    ...
    log_warn("Text property %d of window %#010x has unsupported type: %d", prop, wid, type);
    return false;
  }

  if (format != 8) {
    log_warn("Text property %d of window %#010x has unexpected format: %d", prop, wid, format);
    return false;
  }
  ...
  while (current_offset < length) {
    current_offset += (uint32_t)strnlen(data + current_offset, length - current_offset) + 1;
    nstr += 1;
  }
...
static inline void x_get_server_pictfmts(struct x_connection *c) {
  ...
  g_pictfmts =
      xcb_render_query_pict_formats_reply(c->c, xcb_render_query_pict_formats(c->c), &e);
  if (e || !g_pictfmts) {
...
static xcb_visualid_t attr_pure x_get_visual_for_pictfmt(xcb_render_query_pict_formats_reply_t *r,
  ...
  for (auto screen = xcb_render_query_pict_formats_screens_iterator(r); screen.rem;
    ...
    for (auto depth = xcb_render_pictscreen_depths_iterator(screen.data); depth.rem;
...
xcb_visualid_t x_get_visual_for_depth(struct x_connection *c, uint8_t depth) {
  ...
  for (; screen_it.rem; xcb_screen_next(&screen_it)) {
    xcb_depth_iterator_t depth_it = xcb_screen_allowed_depths_iterator(screen_it.data);
...
x_create_picture_with_pictfmt_and_pixmap(struct x_connection *c,
  ...
  xcb_generic_error_t *e =
      xcb_request_check(c->c, xcb_render_create_picture_checked(
                                  c->c, tmp_picture, pixmap, pictfmt->id, valuemask, buf));
  free(buf);
  if (e) {
...
uint32_t x_create_region(struct x_connection *c, const region_t *reg) {
  ...
  for (int i = 0; i < nrects; i++) {
    xrects[i] = (xcb_rectangle_t){.x = to_i16_checked(rects[i].x1),
  ...
  bool success =
      XCB_AWAIT_VOID(xcb_xfixes_create_region, c->c, ret, to_u32_checked(nrects), xrects);
  free(xrects);
  if (!success) {
    return XCB_NONE;
  }
...
void x_set_picture_clip_region(struct x_connection *c, xcb_render_picture_t pict,
  ...
  xcb_generic_error_t *e = xcb_request_check(
      c->c, xcb_render_set_picture_clip_rectangles_checked(
                c->c, pict, clip_x_origin, clip_y_origin, to_u32_checked(nrects), xrects));
  if (e) {
...
xcb_pixmap_t x_create_pixmap(struct x_connection *c, uint8_t depth, int width, int height) {
  ...
  xcb_void_cookie_t cookie = xcb_create_pixmap_checked(
      c->c, depth, pix, c->screen_info->root, to_u16_checked(width), to_u16_checked(height));
...
xcb_pixmap_t x_get_root_back_pixmap(struct x_connection *c, struct atom *atoms) {
  ...
  for (size_t i = 0; i < ARR_SIZE(root_back_pixmap_atoms); i++) {
    winprop_t prop = x_get_prop(c, c->screen_info->root, root_back_pixmap_atoms[i], 1,
                                XCB_ATOM_PIXMAP, 32);
    if (prop.nitems) {
...
void x_create_convolution_kernel(const conv *kernel, double center,
  ...
  if (!*ret || (*ret)->capacity < kernel->w * kernel->h + 2) {
    free(*ret);
    *ret = cvalloc(sizeof(struct x_convolution_kernel) +
