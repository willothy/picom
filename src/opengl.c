...
bool glx_init(session_t *ps, bool need_render) {
  ...
  if (need_render) {
    ...
    if (Success != glXGetConfig(ps->c.dpy, pvis, GLX_DOUBLEBUFFER, &value) || !value) {
  ...
  if (!psglx->context) {
...
#else
    {
      ...
      f_glXCreateContextAttribsARB p_glXCreateContextAttribsARB =
          (f_glXCreateContextAttribsARB)glXGetProcAddress((const GLubyte *)"glXCreateCont"
                                                                           "extAttribsAR"
                                                                           "B");
      ...
      static const int attrib_list[] = {GLX_CONTEXT_FLAGS_ARB, GLX_CONTEXT_DEBUG_BIT_ARB, None};
      psglx->context =
          p_glXCreateContextAttribsARB(ps->c.dpy, fbconfig, NULL, GL_TRUE, attrib_list);
    }
#endif
...
#ifdef DEBUG_GLX_DEBUG_CONTEXT
    {
      f_DebugMessageCallback p_DebugMessageCallback =
          (f_DebugMessageCallback)glXGetProcAddress((const GLubyte *)"glDebugMessageCallb"
                                                                     "ack");
...
bool glx_init_blur(session_t *ps) {
...

  {
    ...
    for (int i = 0; i < ps->o.blur_kernel_count; ++i) {
      ...
      assert(nele >= 0);
      auto len = strlen(FRAG_SHADER_BLUR_PREFIX) + strlen(sampler_type) + strlen(extension) +
...
#define P_GET_UNIFM_LOC(name, target)                                                      \
  {                                                                                        \
    ...
    if (ppass->target < 0) {                                                               \
      log_error(                                                                           \
          "Failed to get location of %d-th uniform '" name "'. Might be troublesome.", i); \
    }                                                                                      \
  }
...
bool glx_bind_pixmap(session_t *ps, glx_texture_t **pptex, xcb_pixmap_t pixmap, int width,
  ...
  if (!ptex->glpixmap) {
    ...
    if (!width || !height) {
      auto r = xcb_get_geometry_reply(ps->c.c, xcb_get_geometry(ps->c.c, pixmap), NULL);
      if (!r) {
        log_error("Failed to query info of pixmap %#010x.", pixmap);
        return false;
      }
      if (r->depth > OPENGL_MAX_DEPTH) {
        log_error("Requested depth %d higher than %d.", depth, OPENGL_MAX_DEPTH);
        return false;
      }
      depth = r->depth;
      width = r->width;
    ...
    GLenum tex_tgt = 0;
    if (GLX_TEXTURE_2D_BIT_EXT & fbcfg->texture_tgts && ps->psglx->has_texture_non_power_of_two) {
    ...
    ptex->pixmap = pixmap;
    ptex->target = (GLX_TEXTURE_2D_EXT == tex_tgt ? GL_TEXTURE_2D : GL_TEXTURE_RECTANGLE);
    ptex->width = width;
    ptex->height = height;
...
void glx_set_clip(session_t *ps, const region_t *reg) {
  ...
  if (nrects == 1) {
    ...
    glScissor(rects[0].x1, ps->root_height - rects[0].y2, rects[0].x2 - rects[0].x1,
  ...
  */
  ...
  for (int i = 0; i < ps->o.blur_kernel_count; ++i) {
    ...
    if (!last_pass) {
      ...
      glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, tex_scr2, 0);
...
void glx_read_border_pixel(int root_height, int root_width, int x, int y, int width,
  ...
  log_trace("xy(%d, %d), glxy(%d %d) wh(%d %d), border_col(%.2f, %.2f, %.2f, %.2f)", x, y,
            screen_x, screen_y, width, height, (float)ppixel[0], (float)ppixel[1],
            (float)ppixel[2], (float)ppixel[3]);
...
bool glx_round_corners_dst(session_t *ps, struct managed_win *w,
  ...
  if (w->g.border_width > 0) {
    glx_read_border_pixel(ps->root_height, ps->root_width, dx, dy, width, height,
                          &w->border_col[0]);
  }

  {
    ...
    log_trace("ptex: %p wh(%d %d) %d %d", ptex, ptex->width, ptex->height, ptex->target,
              ptex->texture);
    ...
    if (ppass->unifm_borderc >= 0) {
      glUniform4f(ppass->unifm_borderc, w->border_col[0], w->border_col[1],
                  w->border_col[2], w->border_col[3]);
    }
