...
/// Pointer to the global Lua state. TODO: move this to session_t.
lua_State *ps_lua = NULL;
...
enum vblank_callback_action check_render_finish(struct vblank_event *e attr_unused, void *ud) {
  ...
  bool completed = ps->backend_data->ops->last_render_time(ps->backend_data, &render_time);
  if (!completed) {
  ...
  if (ps->o.debug_options.smart_frame_pacing) {
    int render_time_us = (int)(render_time.tv_sec * 1000000L + render_time.tv_nsec / 1000L);
...
collect_vblank_interval_statistics(struct vblank_event *e, void *ud) {
  ...
  if (ps->last_msc < e->msc) {
    if (ps->last_msc_instant != 0) {
      ...
      auto frame_time = (int)((e->ust - ps->last_msc_instant) / frame_count);
      if (frame_count == 1) {
...
void schedule_render(session_t *ps, bool triggered_by_vblank attr_unused) {
  ...
  if (ps->backend_busy) {
    ...
    if (!vblank_scheduler_schedule(ps->vblank_scheduler, reschedule_render_at_vblank, ps)) {
  ...
  if (render_budget >= frame_time) {
    ...
    // render immediately.
    log_verbose("Render budget: %u us >= frame time: %" PRIu32 " us", render_budget, frame_time);
    goto schedule;
  }
...
void queue_redraw(session_t *ps) {
  log_verbose("Queue redraw, render_queued: %d, backend_busy: %d", ps->render_queued,
              ps->backend_busy);
...
static bool run_fade(session_t *ps, struct managed_win **_w, long long steps) {
  auto w = *_w;
  log_trace("Process fading for window %s (%#010x), steps: %lld", w->name, w->base.id, steps);
  ...
  if (steps) {
    if (w->opacity < w->opacity_target) {
      w->opacity =
          clamp(w->opacity + ps->o.fade_in_step * (double)steps, 0.0, w->opacity_target);
    } else {
      w->opacity =
          clamp(w->opacity - ps->o.fade_out_step * (double)steps, w->opacity_target, 1);
    }
...
void update_ewmh_active_win(session_t *ps) {
  ...
  xcb_window_t wid =
      wid_get_prop_window(&ps->c, ps->c.screen_info->root, ps->atoms->a_NET_ACTIVE_WINDOW);
...
static void recheck_focus(session_t *ps) {
  ...
  log_trace("%#010" PRIx32 " (%#010lx \"%s\") focused.", wid, (w ? w->base.id : XCB_NONE),
            (w ? w->name : NULL));
...
static void destroy_backend(session_t *ps) {
  win_stack_foreach_managed_safe(w, &ps->window_stack) {
    ...
    if (ps->backend_data) {
      ...
      if (win_check_flags_any(w, WIN_FLAGS_IMAGES_STALE) && w->state == WSTATE_MAPPED) {
  ...
  if (ps->backend_data) {
    ...
    if (ps->backend_blur_context) {
      ps->backend_data->ops->destroy_blur_context(ps->backend_data, ps->backend_blur_context);
...
static bool initialize_blur(session_t *ps) {
  ...
  ps->backend_blur_context =
      ps->backend_data->ops->create_blur_context(ps->backend_data, ps->o.blur_method, args);
...
static bool initialize_backend(session_t *ps) {
  if (!ps->o.legacy_backends) {
    ...
    ps->backend_data = backend_list[ps->o.backend]->init(ps, session_get_target_window(ps));
    if (!ps->backend_data) {
    ...
    ps->shadow_context =
        ps->backend_data->ops->create_shadow_context(ps->backend_data, ps->o.shadow_radius);
    if (!ps->shadow_context) {
    ...
    HASH_ITER2(ps->shaders, shader) {
      ...
      if (shader->backend_shader == NULL) {
        ...
        if (ps->backend_data->ops->get_shader_attributes) {
          shader->attributes = ps->backend_data->ops->get_shader_attributes(
              ps->backend_data, shader->backend_shader);
        } else {
          shader->attributes = 0;
        }
        log_debug("Shader %s has attributes %" PRIu64, shader->key, shader->attributes);
      }
    }
  ...
  *animation_running = false;
  ...
  win_stack_foreach_managed_safe(w, &ps->window_stack) {
    ...
    if (ps->o.animations && !isnan(w->animation_progress) &&
        w->animation_progress != 1.0 && ps->o.wintype_option[w->window_type].animation != 0
        // NOTE: (willothy): commenting this out made the animations work
        // properly
        // && win_is_mapped_in_x(w)
    ) {
      double neg_displacement_x = w->animation_dest_center_x - w->animation_center_x;
      double neg_displacement_y = w->animation_dest_center_y - w->animation_center_y;
      ...
      double acceleration_x = (animation_stiffness * neg_displacement_x -
                               ps->o.animation_dampening * w->animation_velocity_x) /
                              ps->o.animation_window_mass;
      double acceleration_y = (animation_stiffness * neg_displacement_y -
                               ps->o.animation_dampening * w->animation_velocity_y) /
                              ps->o.animation_window_mass;
      double acceleration_w = (animation_stiffness * neg_displacement_w -
                               ps->o.animation_dampening * w->animation_velocity_w) /
                              ps->o.animation_window_mass;
      double acceleration_h = (animation_stiffness * neg_displacement_h -
      ...
      if (ps->o.animation_clamping) {
        w->animation_center_x =
            clamp(new_animation_x, min2(w->animation_center_x, w->animation_dest_center_x),
                  max2(w->animation_center_x, w->animation_dest_center_x));
        w->animation_center_y =
            clamp(new_animation_y, min2(w->animation_center_y, w->animation_dest_center_y),
                  max2(w->animation_center_y, w->animation_dest_center_y));
        w->animation_w = clamp(new_animation_w, min2(w->animation_w, w->animation_dest_w),
                               max2(w->animation_w, w->animation_dest_w));
        w->animation_h = clamp(new_animation_h, min2(w->animation_h, w->animation_dest_h),
      ...
      bool position_changed = new_animation_x != old_g.x || new_animation_y != old_g.y;
      bool size_changed = new_animation_w != old_g.width || new_animation_h != old_g.height;
  ...
  win_stack_foreach_managed(w, &ps->window_stack) {
    ...
    log_trace("Checking whether window %#010x (%s) should be painted", w->base.id, w->name);
    ...
    if ((w->mode != WMODE_TRANS && !ps->o.force_win_blend) ||
      ...
      if (w->mode == WMODE_SOLID) {
        *tmp = win_get_bounding_shape_global_without_corners_by_val(w);
      } else {
    ...
    if (ps->o.unredir_if_possible && is_highest) {
      if (w->mode == WMODE_SOLID && !ps->o.force_win_blend && win_is_fullscreen(ps, w) &&
  ...
  if (unredir_possible) {
    if (ps->redirected) {
      if (!ps->o.unredir_if_possible_delay || ps->tmout_unredir_hit) {
        ...
        ev_timer_set(&ps->unredir_timer, (double)ps->o.unredir_if_possible_delay / 1000.0, 0);
  ...
  if (ps->backend_data) {
    ...
    if (pixmap != XCB_NONE) {
      xcb_get_geometry_reply_t *r =
          xcb_get_geometry_reply(ps->c.c, xcb_get_geometry(ps->c.c, pixmap), NULL);
      if (!r) {
        goto err;
      }
      ...
      xcb_visualid_t visual = r->depth == ps->c.screen_info->root_depth
      ...
      if (ps->root_image) {
        ps->backend_data->ops->set_image_property(
            ps->backend_data, IMAGE_PROPERTY_EFFECTIVE_SIZE, ps->root_image,
...
static int register_cm(session_t *ps) {
  ...
  for (size_t i = 0; i < ARR_SIZE(prop_atoms); i++) {
    e = xcb_request_check(
        ps->c.c, xcb_change_property_checked(
                     ...
                     prop_is_utf8[i] ? ps->atoms->aUTF8_STRING : XCB_ATOM_STRING, 8,
                     strlen("picom"), "picom"));
    if (e) {
      log_error_x_error(e, "Failed to set window property %d", prop_atoms[i]);
      free(e);
    }
  }
  ...
  e = xcb_request_check(ps->c.c, xcb_change_property_checked(
                                     ps->c.c, XCB_PROP_MODE_REPLACE, ps->reg_win,
                                     get_atom(ps->atoms, "COMPTON_VERSION"), XCB_ATOM_STRING,
                                     8, (uint32_t)strlen(PICOM_VERSION), PICOM_VERSION));
  if (e) {
  ...
  if (!ps->o.no_x_selection) {
    ...
    xcb_get_selection_owner_reply_t *reply =
        xcb_get_selection_owner_reply(ps->c.c, xcb_get_selection_owner(ps->c.c, atom), NULL);
...
static bool init_overlay(session_t *ps) {
  ...
  if (ps->overlay != XCB_NONE) {
    ...
    // compiz-0.8.8
    if (!XCB_AWAIT_VOID(xcb_shape_mask, ps->c.c, XCB_SHAPE_SO_SET, XCB_SHAPE_SK_BOUNDING,
                        ps->overlay, 0, 0, 0)) {
      log_fatal("Failed to set the bounding shape of overlay, giving "
                "up.");
      return false;
    }
    if (!XCB_AWAIT_VOID(xcb_shape_rectangles, ps->c.c, XCB_SHAPE_SO_SET, XCB_SHAPE_SK_INPUT,
...
static bool init_debug_window(session_t *ps) {
  ...
  auto err = xcb_request_check(
      ps->c.c, xcb_create_colormap_checked(ps->c.c, XCB_COLORMAP_ALLOC_NONE, colormap,
  ...
  err = xcb_request_check(
      ...
      xcb_create_window_checked(
          ps->c.c, (uint8_t)ps->c.screen_info->root_depth, ps->debug_window,
          ps->c.screen_info->root, 0, 0, to_u16_checked(ps->root_width),
          to_u16_checked(ps->root_height), 0, XCB_WINDOW_CLASS_INPUT_OUTPUT,
          ps->c.screen_info->root_visual, XCB_CW_COLORMAP, (uint32_t[]){colormap, 0}));
  if (err) {
    goto err_out;
  }
...
static bool redirect_start(session_t *ps) {
  ...
  if (ps->present_exists && ps->frame_pacing) {
    ...
    enum vblank_scheduler_type scheduler_type = choose_vblank_scheduler(ps->drivers);
    if (ps->o.debug_options.force_vblank_scheduler != LAST_VBLANK_SCHEDULER) {
      scheduler_type = (enum vblank_scheduler_type)ps->o.debug_options.force_vblank_scheduler;
    }
    ...
    vblank_scheduler_schedule(ps->vblank_scheduler, collect_vblank_interval_statistics, ps);
...
static void handle_pending_updates(EV_P_ struct session *ps) {
  if (ps->pending_updates) {
    ...
    {
      auto r = xcb_get_input_focus_reply(ps->c.c, xcb_get_input_focus(ps->c.c), NULL);
  ...
  *ps = s_def;
  ...
  auto e = xcb_request_check(
      ps->c.c, xcb_change_window_attributes_checked(
                   ...
                   (const uint32_t[]){
                       XCB_EVENT_MASK_SUBSTRUCTURE_NOTIFY | XCB_EVENT_MASK_EXPOSURE |
...
#undef SET_WM_TYPE_ATOM
  ...
  if (ps->o.legacy_backends) {
    ps->shadow_context = (void *)gaussian_kernel_autodetect_deviation(ps->o.shadow_radius);
  ...
  if (ext_info && ext_info->present) {
    auto r = xcb_present_query_version_reply(
        ps->c.c,
        xcb_present_query_version(ps->c.c, XCB_PRESENT_MAJOR_VERSION, XCB_PRESENT_MINOR_VERSION),
        NULL);
    if (r) {
  ...
  if (ext_info && ext_info->present) {
    ...
    auto r = xcb_sync_initialize_reply(
        ps->c.c,
        xcb_sync_initialize(ps->c.c, XCB_SYNC_MAJOR_VERSION, XCB_SYNC_MINOR_VERSION), NULL);
    if (r && (r->major_version > 3 || (r->major_version == 3 && r->minor_version >= 1))) {
  ...
  if (ps->xsync_exists) {
    ...
    e = xcb_request_check(ps->c.c, xcb_sync_create_fence_checked(
  ...
  xcb_query_tree_reply_t *query_tree_reply =
      xcb_query_tree_reply(ps->c.c, xcb_query_tree(ps->c.c, ps->c.screen_info->root), NULL);
