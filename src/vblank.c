...
static bool check_sgi_video_sync_extension(Display *dpy, int screen) {
  ...
  glXWaitVideoSyncSGI =
      (PFNGLXWAITVIDEOSYNCSGIPROC)(void *)glXGetProcAddress((const GLubyte *)"glXWaitVide"
                                                                             "oSyncSGI");
...
static void *sgi_video_sync_thread(void *data) {
  ...
  GLXFBConfig *cfg_ = glXChooseFBConfig(
      dpy, screen,
      (int[]){GLX_RENDER_TYPE, GLX_RGBA_BIT, GLX_DRAWABLE_TYPE, GLX_WINDOW_BIT, 0}, &ncfg);
  GLXContext ctx = NULL;
  ...
  dummy = XCreateWindow(dpy, root, 0, 0, 1, 1, 0, depth, InputOutput, visual, CWColormap,
                        &attributes);
  ...
  while (self->running) {
    if (!self->base.vblank_event_requested) {
      pthread_cond_wait(&self->vblank_requested_cnd, &self->vblank_requested_mtx);
      continue;
    }
    ...
    atomic_store(&self->current_ust, (uint64_t)(now.tv_sec * 1000000 + now.tv_nsec / 1000));
...
static bool sgi_video_sync_scheduler_init(struct vblank_scheduler *base) {
  ...
  if (args.start_status != 0) {
    log_fatal("Failed to start sgi_video_sync_thread, error code: %d", args.start_status);
    succeeded = false;
  } else {
...
static bool present_vblank_scheduler_init(struct vblank_scheduler *base) {
  ...
  auto select_input = xcb_present_select_input(base->c->c, self->event_id, base->target_window,
...
static void handle_present_complete_notify(struct present_vblank_scheduler *self,
  ...
  if (event_is_invalid) {
    log_debug("Invalid PresentCompleteNotify event, %" PRIu64 " %" PRIu64, cne->msc, cne->ust);
...
bool vblank_scheduler_schedule(struct vblank_scheduler *self,
  ...
  if (self->callback_count == self->callback_capacity) {
    size_t new_capacity = self->callback_capacity ? self->callback_capacity * 2 : 1;
    void *new_buffer = realloc(self->callbacks, new_capacity * sizeof(*self->callbacks));
    if (!new_buffer) {
      return false;
    }
...
vblank_scheduler_invoke_callbacks(struct vblank_scheduler *self, struct vblank_event *event) {
  ...
  memset(self->callbacks + write_head, 0, (count - write_head) * sizeof(*self->callbacks));
