...
static void win_update_focused(session_t *ps, struct managed_win *w) {
  if (UNSET != w->focused_force) {
    ...
    // windows specially
    if (ps->o.wintype_option[w->window_type].focus || (ps->o.mark_wmwin_focused && w->wmwin) ||
    ...
    // its group is
    if (ps->o.track_leader && ps->active_leader && win_get_leader(ps, w) == ps->active_leader) {
      w->focused = true;
    }
  }
}
...
static inline bool win_bind_pixmap(struct backend_base *b, struct managed_win *w) {
  ...
  w->win_image = b->ops->bind_pixmap(b, pixmap, x_get_visual_info(b->c, w->a.visual), true);
  if (!w->win_image) {
...
static inline bool win_bounding_shaped(const session_t *ps, xcb_window_t wid) {
  if (ps->shape_exists) {
    ...
    reply =
        xcb_shape_query_extents_reply(ps->c.c, xcb_shape_query_extents(ps->c.c, wid), NULL);
...
wid_get_opacity_prop(session_t *ps, xcb_window_t wid, opacity_t def, opacity_t *out) {
  ...
  *out = def;
  ...
  winprop_t prop =
      x_get_prop(&ps->c, wid, ps->atoms->a_NET_WM_WINDOW_OPACITY, 1L, XCB_ATOM_CARDINAL, 32);
...
void win_update_prop_shadow_raw(session_t *ps, struct managed_win *w) {
  winprop_t prop =
      x_get_prop(&ps->c, w->base.id, ps->atoms->a_COMPTON_SHADOW, 1, XCB_ATOM_CARDINAL, 32);
...
void win_on_win_size_change(session_t *ps, struct managed_win *w) {
  log_trace("Window %#010x (%s) size changed, was %dx%d, now %dx%d", w->base.id, w->name,
            w->widthb, w->heightb, w->g.width + w->g.border_width * 2,
...
void win_unmark_client(session_t *ps, struct managed_win *w) {
  ...
  log_debug("Detaching client window %#010x from frame %#010x (%s)", client, w->base.id,
            w->name);
...
struct win *fill_win(session_t *ps, struct win *w) {
  ...
  xcb_get_window_attributes_cookie_t acookie = xcb_get_window_attributes(ps->c.c, w->id);
  ...
  *new = win_def;
  ...
  e = xcb_request_check(ps->c.c, xcb_damage_create_checked(ps->c.c, new->damage, w->id,
  ...
  win_set_flags(new, WIN_FLAGS_CLIENT_STALE | WIN_FLAGS_SIZE_STALE | WIN_FLAGS_POSITION_STALE |
...
void win_update_leader(session_t *ps, struct managed_win *w) {
  ...
  if (ps->o.detect_transient && !leader) {
    leader = wid_get_prop_window(&ps->c, w->client_win, ps->atoms->aWM_TRANSIENT_FOR);
  }

  if (ps->o.detect_client_leader && !leader) {
    leader = wid_get_prop_window(&ps->c, w->client_win, ps->atoms->aWM_CLIENT_LEADER);
  }
...
static void win_on_focus_change(session_t *ps, struct managed_win *w) {
  ...
  if (ps->o.track_leader) {
    ...
    else if (!win_is_focused_raw(ps, w) && leader && leader == ps->active_leader &&
...
void win_extents(const struct managed_win *w, region_t *res) {
  ...
  if (w->shadow) {
    ...
    pixman_region32_union_rect(res, res, w->g.x + w->shadow_dx, w->g.y + w->shadow_dy,
...
void win_update_bounding_shape(session_t *ps, struct managed_win *w) {
  ...
  while (w->bounding_shaped) {
    ...
    xcb_shape_get_rectangles_reply_t *r = xcb_shape_get_rectangles_reply(
        ps->c.c, xcb_shape_get_rectangles(ps->c.c, w->base.id, XCB_SHAPE_SK_BOUNDING), NULL);
...
void win_update_opacity_prop(session_t *ps, struct managed_win *w) {
  ...
  // get client opacity
  w->has_opacity_prop = wid_get_opacity_prop(ps, w->client_win, OPAQUE, &w->opacity_prop);
}
...
void win_update_frame_extents(session_t *ps, struct managed_win *w, xcb_window_t client) {
  winprop_t prop =
      x_get_prop(&ps->c, client, ps->atoms->a_NET_FRAME_EXTENTS, 4L, XCB_ATOM_CARDINAL, 32);
  ...
  if (prop.nitems == 4) {
    ...
    const bool changed =
        w->frame_extents.left != extents[0] || w->frame_extents.right != extents[1] ||
        w->frame_extents.top != extents[2] || w->frame_extents.bottom != extents[3];
  ...
  for (int i = 0; i < monitors->count; i++) {
    ...
    if (((e->x1 <= mw->g.x || e->x1 <= mw->pending_g.x) &&
      ...
      log_debug("Window %#010x (%s), %dx%d+%dx%d, is entirely on the "
                "monitor %d (%dx%d+%dx%d)",
                mw->base.id, mw->name, mw->g.x, mw->g.y, mw->widthb, mw->heightb, i,
...
void win_update_opacity_target(session_t *ps, struct managed_win *w) {
  ...
  if (w->state == WSTATE_MAPPED) {
    ...
    log_debug("Window %#010x (%s) opacity %f, opacity target %f, set "
              "old target %f",
              w->base.id, w->name, w->opacity, w->opacity_target, w->opacity_target_old);
...
void win_clear_flags(struct managed_win *w, uint64_t flags) {
  ...
  if (unlikely(w->state == WSTATE_DESTROYING)) {
    log_warn("Flags cleared on a destroyed window %#010x (%s)", w->base.id, w->name);
    return;
  }
...
void win_set_properties_stale(struct managed_win *w, const xcb_atom_t *props, int nprops) {
  ...
  if (new_capacity > w->stale_props_capacity) {
    w->stale_props = realloc(w->stale_props, new_capacity * sizeof(*w->stale_props));
  ...
  for (int i = 0; i < nprops; i++) {
    w->stale_props[props[i] / bits_per_element] |= 1UL << (props[i] % bits_per_element);
  }
...
bool win_is_fullscreen(const session_t *ps, const struct managed_win *w) {
  if (!ps->o.no_ewmh_fullscreen && win_is_fullscreen_xcb(ps->c.c, ps->atoms, w->client_win)) {
    return true;
  }
...
bool win_is_bypassing_compositor(const session_t *ps, const struct managed_win *w) {
  ...
  auto prop = x_get_prop(&ps->c, w->client_win, ps->atoms->a_NET_WM_BYPASS_COMPOSITOR, 1L,
