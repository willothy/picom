...
#define cdbus_reply_err(ps, srcmsg, err_name, err_format, ...)                           \
  cdbus_reply_errm((ps), dbus_message_new_error_printf((srcmsg), (err_name),             \
                                                       (err_format), ##__VA_ARGS__))
...
bool cdbus_init(session_t *ps, const char *uniq) {
  ...
  // Request service name
  {
    ...
    int ret =
        dbus_bus_request_name(cd->dbus_conn, service, DBUS_NAME_FLAG_DO_NOT_QUEUE, &err);
  ...
  if (!dbus_connection_set_timeout_functions(cd->dbus_conn, cdbus_callback_add_timeout,
  ...
  dbus_connection_register_object_path(cd->dbus_conn, CDBUS_OBJECT_NAME,
                                       (DBusObjectPathVTable[]){{NULL, cdbus_process}}, ps);
  dbus_connection_register_fallback(cd->dbus_conn, CDBUS_OBJECT_NAME "/windows",
                                    (DBusObjectPathVTable[]){{NULL, cdbus_process_windows}},
                                    ps);
  return true;
fail:
  ps->dbus_data = NULL;
...
cdbus_append_wid_variant(session_t *ps attr_unused, DBusMessage *msg, const void *data) {
  ...
  if (!dbus_message_iter_open_container(&it, DBUS_TYPE_VARIANT, CDBUS_TYPE_WINDOW_STR, &it2)) {
    return false;
  }
...
static bool cdbus_apdarg_wids(session_t *ps, DBusMessage *msg, const void *data attr_unused) {
  ...
  // Append arguments
  if (!dbus_message_append_args(msg, DBUS_TYPE_ARRAY, CDBUS_TYPE_WINDOW, &arr, count,
                                DBUS_TYPE_INVALID)) {
    log_error("Failed to append argument.");
    free(arr);
    return false;
...
cdbus_process_window_property_get(session_t *ps, DBusMessage *msg, cdbus_window_t wid) {
  ...
  if (!dbus_message_get_args(msg, &err, DBUS_TYPE_STRING, &interface, DBUS_TYPE_STRING,
...
#define cdbus_m_win_get_do(tgt, member, apdarg_func)                                     \
  ...
  if (!strcmp("Mapped", target)) {
    cdbus_reply(ps, msg, cdbus_append_bool_variant, (bool[]){win_is_mapped_in_x(w)});
    return true;
  }
  ...
  if (!strcmp("Next", target)) {
    ...
    if (!list_node_is_last(&ps->window_stack, &w->base.stack_neighbour)) {
      next_id = list_entry(w->base.stack_neighbour.next, struct win, stack_neighbour)->id;
    }
  ...
  if (!strcmp("RawFocused", target)) {
    cdbus_reply(ps, msg, cdbus_append_bool_variant, (bool[]){win_is_focused_raw(ps, w)});
    return true;
  }
  ...
  if (!strcmp("next", target)) {
    cdbus_reply_wid(
        ...
        (list_node_is_last(&ps->window_stack, &w->base.stack_neighbour)
             ? 0
             : list_entry(w->base.stack_neighbour.next, struct win, stack_neighbour)->id));
    return true;
  }
...
finished:
  if (!handled && dbus_message_get_type(msg) == DBUS_MESSAGE_TYPE_METHOD_CALL &&
      !dbus_message_get_no_reply(msg)) {
    handled = cdbus_reply_err(ps, msg, CDBUS_ERROR_UNKNOWN, CDBUS_ERROR_UNKNOWN_S);
  }
  return handled ? DBUS_HANDLER_RESULT_HANDLED : DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
}
