...
static inline void ev_focus_in(session_t *ps, xcb_focus_in_event_t *ev) {
  log_debug("{ mode: %s, detail: %s }\n", ev_focus_mode_name(ev), ev_focus_detail_name(ev));
...
static void configure_win(session_t *ps, xcb_configure_notify_event_t *ce) {
  ...
  bool size_changed = mw->pending_g.width != ce->width || mw->pending_g.height != ce->height ||
  ...
  if (position_changed || size_changed) {
    ...
    if (position_changed) {
      log_trace("Window position changed, %dx%d -> %dx%d", mw->g.x, mw->g.y, ce->x, ce->y);
      mw->pending_g.x = ce->x;
      mw->pending_g.y = ce->y;
    ...
    if (size_changed) {
      log_trace("Window size changed, %dx%d -> %dx%d", mw->g.width, mw->g.height,
                ce->width, ce->height);
...
static inline void ev_map_notify(session_t *ps, xcb_map_notify_event_t *ev) {
  ...
  if (ps->overlay && ev->window == ps->overlay && !ps->redirected) {
    ...
    auto e = xcb_request_check(ps->c.c, xcb_unmap_window_checked(ps->c.c, ps->overlay));
    if (e) {
...
static inline void ev_reparent_notify(session_t *ps, xcb_reparent_notify_event_t *ev) {
  log_debug("Window %#010x has new parent: %#010x, override_redirect: %d", ev->window,
    ...
    if (!wid_has_prop(ps, ev->window, ps->atoms->aWM_STATE)) {
      ...
      if (w_real_top && w_real_top->state != WSTATE_UNMAPPED &&
        ...
        if (!w_real_top) {
          ...
          log_debug("parent %#010x (%s) is in state %d", w_real_top->base.id,
                    w_real_top->name, w_real_top->state);
        }
      }
    }
  }
...
static inline void ev_expose(session_t *ps, xcb_expose_event_t *ev) {
  if (ev->window == ps->c.screen_info->root || (ps->overlay && ev->window == ps->overlay)) {
    int more = ev->count + 1;
...
static inline void ev_property_notify(session_t *ps, xcb_property_notify_event_t *ev) {
  if (unlikely(log_get_level_tls() <= LOG_LEVEL_TRACE)) {
    ...
    xcb_get_atom_name_reply_t *reply =
        xcb_get_atom_name_reply(ps->c.c, xcb_get_atom_name(ps->c.c, ev->atom), NULL);
    const char *name = "?";
    int name_len = 1;
    if (reply) {
...
static inline void repair_win(session_t *ps, struct managed_win *w) {
  ...
  if (!w->ever_damaged) {
    ...
    if (!ps->o.show_all_xerrors) {
      set_ignore_cookie(&ps->c, xcb_damage_subtract(ps->c.c, w->damage, XCB_NONE, XCB_NONE));
    }
  } else {
    if (!ps->o.show_all_xerrors) {
      set_ignore_cookie(
          &ps->c, xcb_damage_subtract(ps->c.c, w->damage, XCB_NONE, ps->damaged_region));
    }
    x_fetch_region(&ps->c, ps->damaged_region, &parts);
    pixman_region32_translate(&parts, w->g.x + w->g.border_width, w->g.y + w->g.border_width);
  }
...
void ev_handle(session_t *ps, xcb_generic_event_t *ev) {
  ...
  if (ev->response_type != ps->damage_event + XCB_DAMAGE_NOTIFY) {
    log_debug("event %10.10s serial %#010x window %#010x \"%s\"", ev_name(ps, ev),
              ev->full_sequence, wid, ev_window_name(ps, wid));
  } else {
    log_trace("event %10.10s serial %#010x window %#010x \"%s\"", ev_name(ps, ev),
              ev->full_sequence, wid, ev_window_name(ps, wid));
  }
  ...
  case ConfigureNotify:
    ...
    break;
  case DestroyNotify: ev_destroy_notify(ps, (xcb_destroy_notify_event_t *)ev); break;
  ...
  case ReparentNotify: ev_reparent_notify(ps, (xcb_reparent_notify_event_t *)ev); break;
  case CirculateNotify:
    ev_circulate_notify(ps, (xcb_circulate_notify_event_t *)ev);
    break;
  case Expose: ev_expose(ps, (xcb_expose_event_t *)ev); break;
  case PropertyNotify: ev_property_notify(ps, (xcb_property_notify_event_t *)ev); break;
  case SelectionClear: ev_selection_clear(ps, (xcb_selection_clear_event_t *)ev); break;
