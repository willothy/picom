...
parse_cfg_condlst_corner(options_t *opt, const config_t *pcfg, const char *name) {
  ...
  if (setting) {
    ...
    // Treat it as a single pattern if it's a string
      ...
      if (!parse_numeric_window_rule(&opt->corner_radius_rules,
                                     config_setting_get_string(setting), 0, INT_MAX)) {
        exit(1);
      }
    }
  }
  ...
  if (setting) {
    ...
    if (config_setting_is_array(setting)) {
      ...
      while (i--) {
        if (!parse_numeric_window_rule(
                &opt->opacity_rules, config_setting_get_string_elem(setting, i), 0, 100)) {
          exit(1);
        }
      }
    }
    // Treat it as a single pattern if it's a string
      ...
      if (!parse_numeric_window_rule(&opt->opacity_rules,
                                     config_setting_get_string(setting), 0, 100)) {
        exit(1);
      }
    }
  }
    ...
    if (config_setting_is_array(setting)) {
      ...
      while (i--) {
        if (!parse_rule_window_shader(&opt->window_shader_fg_rules,
                                      config_setting_get_string_elem(setting, i), include_dir)) {
          exit(1);
        }
      }
    }
...
char *parse_config_libconfig(options_t *opt, const char *config_file, bool *shadow_enable,
  ...
  if (config_lookup_string(&cfg, "blur-kern", &sval)) {
    opt->blur_kerns = parse_blur_kern_lst(sval, conv_kern_hasneg, &opt->blur_kernel_count);
    if (!opt->blur_kerns) {
  ...
  if (config_lookup_string(&cfg, "glx-swap-method", &sval)) {
    ...
    log_error("glx-swap-method has been removed, your setting "
              "\"%s\" should be %s.",
              sval, !should_remove ? "replaced by `use-damage = true`" : "removed");
    goto err;
  }
  // --use-damage
  ...
  parse_cfg_condlst_shader(opt, &cfg, "window-shader-fg-rule", config_get_include_dir(&cfg));
  ...
  for (wintype_t i = 0; i < NUM_WINTYPES; ++i) {
    parse_wintype_config(&cfg, WINTYPES[i], &opt->wintype_option[i], &winopt_mask[i]);
  }
