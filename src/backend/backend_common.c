...
xcb_image_t *make_shadow(struct x_connection *c, const conv *kernel, double opacity,
  ...
  ximage = xcb_image_create_native(c->c, to_u16_checked(swidth), to_u16_checked(sheight),
  ...
  if (width < r * 2 && height < r * 2) {
    for (int y = 0; y < sheight; y++) {
      for (int x = 0; x < swidth; x++) {
        double sum = sum_kernel_normalized(kernel, d - x - 1, d - y - 1, width, height);
  ...
  if (height < r * 2) {
    ...
    for (int y = 0; y < sheight; y++) {
      for (int x = 0; x < r * 2; x++) {
        double sum = sum_kernel_normalized(kernel, d - x - 1, d - y - 1, d, height) *
                     255.0 * opacity;
    ...
    for (int y = 0; y < sheight; y++) {
      double sum = sum_kernel_normalized(kernel, 0, d - y - 1, d, height) * 255.0 * opacity;
      memset(&data[y * sstride + r * 2], (uint8_t)sum, (size_t)(width - 2 * r));
    }
    return ximage;
  }
  if (width < r * 2) {
    ...
    for (int y = 0; y < r * 2; y++) {
      for (int x = 0; x < swidth; x++) {
        double sum =
            sum_kernel_normalized(kernel, d - x - 1, d - y - 1, width, d) * 255.0 * opacity;
        data[y * sstride + x] = (uint8_t)sum;
        data[(sheight - y - 1) * sstride + x] = (uint8_t)sum;
      }
    }
    for (int x = 0; x < swidth; x++) {
      double sum = sum_kernel_normalized(kernel, d - x - 1, 0, width, d) * 255.0 * opacity;
...
bool build_shadow(struct x_connection *c, double opacity, const int width,
  ...
  shadow_pixmap_argb = x_create_pixmap(c, 32, shadow_image->width, shadow_image->height);
  ...
  shadow_picture = x_create_picture_with_standard_and_pixmap(c, XCB_PICT_STANDARD_A_8,
                                                             shadow_pixmap, 0, NULL);
  ...
  for (uint32_t row = 0; row < shadow_image->height; row += maximum_row) {
    ...
    xcb_put_image(c->c, (uint8_t)shadow_image->format, shadow_pixmap, gc, shadow_image->width,
                  batch_height, 0, to_i16_checked(row), 0, shadow_image->depth,
                  shadow_image->stride * batch_height, shadow_image->data + offset);
  }
...
void *default_render_shadow(backend_t *backend_data, int width, int height,
  ...
  if (!build_shadow(backend_data->c, color.alpha, width, height, kernel, shadow_pixel,
                    &shadow, &pict)) {
...
default_create_shadow_context(backend_t *backend_data attr_unused, double radius) {
  auto ret = (struct backend_shadow_context *)gaussian_kernel_autodetect_deviation(radius);
...
generate_gaussian_blur_kernel(struct gaussian_blur_args *args, int *kernel_count) {
  ...
  for (int i = 0; i <= args->size; i++) {
    ret[0]->data[i] = ret[0]->data[r - i - 1] =
        ...
        exp(-(args->size - i) * (args->size - i) / (2 * args->deviation * args->deviation));
...
struct conv **generate_blur_kernel(enum blur_method method, void *args, int *kernel_count) {
  ...
  case BLUR_METHOD_GAUSSIAN: return generate_gaussian_blur_kernel(args, kernel_count);
  default: break;
  }
  return NULL;
}
