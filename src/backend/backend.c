...
static void process_window_for_painting(session_t *ps, struct managed_win *w,
  ...
  region_t reg_bound_local;
  {
    ...
    pixman_region32_intersect(&reg_visible_local, &reg_visible_local, &reg_bound_local);
  }

  auto new_img =
      ps->backend_data->ops->clone_image(ps->backend_data, win_image, &reg_visible_local);
  ...
  ps->backend_data->ops->set_image_property(ps->backend_data, IMAGE_PROPERTY_OPACITY,
                                            new_img, &alpha);
  ps->backend_data->ops->image_op(ps->backend_data, IMAGE_OP_APPLY_ALPHA, new_img, &reg_frame,
                                  &reg_visible_local, (double[]){w->frame_opacity});
  pixman_region32_fini(&reg_frame);
  ps->backend_data->ops->compose(ps->backend_data, new_img, window_coord, NULL,
                                 dest_coord, reg_paint_in_bound, reg_visible, true);
...
bool paint_all_new(session_t *ps, struct managed_win *const t) {
  ...
  if (ps->o.blur_method != BLUR_METHOD_NONE && ps->backend_data->ops->get_blur_size) {
    ...
    ps->backend_data->ops->get_blur_size(ps->backend_blur_context, &blur_width, &blur_height);
    ...
    resize_region_in_place(&reg_damage, blur_width * resize_factor, blur_height * resize_factor);
    reg_paint =
        resize_region(&reg_damage, blur_width * resize_factor, blur_height * resize_factor);
  ...
  if (ps->next_render > 0) {
    log_verbose("Render schedule deviation: %ld us (%s) %" PRIu64 " %ld",
                ...
                after_damage_us < ps->next_render ? "early" : "late", after_damage_us,
                ps->next_render);
  ...
  if (ps->root_image) {
    ps->backend_data->ops->compose(ps->backend_data, ps->root_image, (coord_t){0}, NULL,
                                   (coord_t){.x = ps->root_width, .y = ps->root_height},
                                   &reg_paint, &reg_visible, true);
  } else {
    ps->backend_data->ops->fill(ps->backend_data, (struct color){0, 0, 0, 1}, &reg_paint);
  }
  ...
  for (struct managed_win *w = t; w; w = w->prev_trans) {
    ...
    if (w->blur_background &&
      ...
      if (w->opacity < (1.0 / MAX_ALPHA)) {
      ...
      } else if (w->state == WSTATE_UNMAPPING || w->state == WSTATE_DESTROYING) {
        ...
        if (w->opacity < w->opacity_target && w->opacity_target_old < (1.0 / MAX_ALPHA)) {
          // Gradually increase the blur intensity during
          // fading in.
          assert(w->opacity <= w->opacity_target);
          blur_opacity = w->opacity / w->opacity_target;
        } else if (w->opacity > w->opacity_target && w->opacity_target < (1.0 / MAX_ALPHA)) {
          // Gradually decrease the blur intensity during
          // fading out.
          assert(w->opacity <= w->opacity_target_old);
          blur_opacity = w->opacity / w->opacity_target_old;
      ...
      if (real_win_mode == WMODE_TRANS || ps->o.force_win_blend) {
        ...
        if (ps->o.transparent_clipping) {
          ...
          pixman_region32_intersect(&reg_blur, &reg_blur, &reg_visible);
        }
        ps->backend_data->ops->blur(ps->backend_data, blur_opacity, ps->backend_blur_context,
    ...
    if (w->shadow) {
      ...
      if (pixman_region32_not_empty(&ps->shadow_exclude_reg)) {
        pixman_region32_subtract(&reg_shadow, &reg_shadow, &ps->shadow_exclude_reg);
      }
      if (pixman_region32_not_empty(&reg_shadow_clip)) {
        pixman_region32_subtract(&reg_shadow, &reg_shadow, &reg_shadow_clip);
      }
      ...
      if (ps->o.transparent_clipping) {
        // ref: <transparent-clipping-note>
        pixman_region32_intersect(&reg_shadow, &reg_shadow, &reg_visible);
      }

      assert(w->shadow_image);
      ps->backend_data->ops->set_image_property(ps->backend_data, IMAGE_PROPERTY_OPACITY,
                                                w->shadow_image, &w->opacity);
      coord_t shadow_coord = {.x = w->g.x + w->shadow_dx, .y = w->g.y + w->shadow_dy};
      ...
      if (!ps->o.wintype_option[w->window_type].full_shadow) {
        ...
        if (w->mask_image) {
          ...
          ps->backend_data->ops->set_image_property(
              ps->backend_data, IMAGE_PROPERTY_INVERTED, inverted_mask, (bool[]){true});
        }
      }
      ps->backend_data->ops->compose(ps->backend_data, w->shadow_image, shadow_coord,
                                     inverted_mask, window_coord, &reg_shadow,
                                     &reg_visible, false);
      if (inverted_mask) {
        ps->backend_data->ops->set_image_property(
            ps->backend_data, IMAGE_PROPERTY_INVERTED, inverted_mask, (bool[]){false});
      }
    ...
    // Update image properties
    {
      ...
      ps->backend_data->ops->set_image_property(ps->backend_data, IMAGE_PROPERTY_MAX_BRIGHTNESS,
                                                w->win_image, &ps->o.max_brightness);
      ps->backend_data->ops->set_image_property(ps->backend_data, IMAGE_PROPERTY_INVERTED,
                                                w->win_image, &w->invert_color);
      ps->backend_data->ops->set_image_property(
          ps->backend_data, IMAGE_PROPERTY_DIM_LEVEL, w->win_image, &dim_opacity);
      ps->backend_data->ops->set_image_property(ps->backend_data, IMAGE_PROPERTY_OPACITY,
                                                w->win_image, &w->opacity);
      ps->backend_data->ops->set_image_property(ps->backend_data, IMAGE_PROPERTY_CORNER_RADIUS,
                                                w->win_image, (double[]){w->corner_radius});
      if (w->corner_radius) {
        int border_width = w->g.border_width;
        if (border_width == 0) {
          // Some WM has borders implemented as WM frames
          border_width =
              min3(w->frame_extents.left, w->frame_extents.right, w->frame_extents.bottom);
        }
        ps->backend_data->ops->set_image_property(
            ps->backend_data, IMAGE_PROPERTY_BORDER_WIDTH, w->win_image, &border_width);
        if (w->old_win_image) {
          // TODO(dccsillag): explain why the following is
          // "necessary"
          double zero = 0.0;
          ps->backend_data->ops->set_image_property(
              ps->backend_data, IMAGE_PROPERTY_BORDER_WIDTH, w->old_win_image, &zero);
          ps->backend_data->ops->set_image_property(
              ps->backend_data, IMAGE_PROPERTY_CORNER_RADIUS, w->old_win_image, &zero);
        }
      }
    ...
    if (w->frame_opacity == 1 && !is_animating) {
      ps->backend_data->ops->compose(ps->backend_data, w->win_image, window_coord, NULL,
                                     dest_coord, &reg_paint_in_bound, &reg_visible, true);
    } else {
      if (is_animating && w->old_win_image) {
        ...
        bool resizing = w->g.width != w->pending_g.width || w->g.height != w->pending_g.height;
        ...
        if (resizing && (!is_focused || !w->opacity_is_set)) {
          process_window_for_painting(ps, w, w->old_win_image,
                                      1.0 - w->animation_progress, &reg_bound,
                                      &reg_visible, &reg_paint, &reg_paint_in_bound);
        }
      } else {
        process_window_for_painting(ps, w, w->win_image, 1.0, &reg_bound, &reg_visible,
