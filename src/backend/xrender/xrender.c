...
make_rounded_corner_cache(struct x_connection *c, xcb_render_picture_t src, int width,
  ...
  auto picture =
      x_create_picture_with_standard(c, width, height, XCB_PICT_STANDARD_ARGB_32, 0, NULL);
  if (picture == XCB_NONE) {
    return NULL;
  }
...
static xcb_render_picture_t process_mask(struct _xrender_data *xd, struct xrender_image *mask,
  ...
  *allocated = true;
  ...
  xcb_render_composite(xd->base.c->c, XCB_RENDER_PICT_OP_SRC, inner->pict, XCB_NONE, ret,
  ...
  if (mask->base.corner_radius != 0) {
    if (mask->rounded_rectangle == NULL) {
      mask->rounded_rectangle =
          make_rounded_corner_cache(xd->base.c, xd->white_pixel, inner->width - 2,
                                    inner->height - 2, (int)mask->base.corner_radius);
    }
    xcb_render_composite(xd->base.c->c, XCB_RENDER_PICT_OP_IN_REVERSE,
                         mask->rounded_rectangle->p, XCB_NONE, ret, 0, 0, 0, 0, 1, 1,
                         (uint16_t)(tmpw - 2), (uint16_t)(tmph - 2));
  }

  if (mask->base.color_inverted) {
    xcb_render_composite(xd->base.c->c, XCB_RENDER_PICT_OP_XOR, xd->white_pixel, XCB_NONE,
                         ret, 0, 0, 0, 0, 0, 0, tmpw, tmph);
  }

  if (alpha_pict != XCB_NONE) {
    xcb_render_composite(xd->base.c->c, XCB_RENDER_PICT_OP_SRC, ret, alpha_pict, ret, 0,
                         0, 0, 0, 0, 0, to_u16_checked(inner->width),
...
compose_impl(struct _xrender_data *xd, struct xrender_image *xrimg, coord_t dst,
  ...
  if (mask != NULL) {
    mask_pict =
        process_mask(xd, mask, img->opacity < 1.0 ? mask_pict : XCB_NONE, &mask_allocated);
  }
  ...
  if (img->corner_radius != 0 && xrimg->rounded_rectangle == NULL) {
    xrimg->rounded_rectangle = make_rounded_corner_cache(
        xd->base.c, xd->white_pixel, inner->width, inner->height, (int)img->corner_radius);
  }
  if (((img->color_inverted || img->dim != 0) && has_alpha) || img->corner_radius != 0) {
    ...
    auto visual = (img->corner_radius != 0 && inner->depth != 32)
                      ? x_get_visual_for_standard(xd->base.c, XCB_PICT_STANDARD_ARGB_32)
                      : inner->visual;
    auto tmp_pict = x_create_picture_with_visual(xd->base.c, inner->width, inner->height,
                                                 visual, 0, NULL);
    ...
    if (img->color_inverted) {
      if (inner->has_alpha) {
        auto tmp_pict2 =
            x_create_picture_with_visual(xd->base.c, tmpw, tmph, inner->visual, 0, NULL);
        xcb_render_composite(xd->base.c->c, XCB_RENDER_PICT_OP_SRC, tmp_pict, XCB_NONE,
                             tmp_pict2, 0, 0, 0, 0, 0, 0, tmpw, tmph);

        xcb_render_composite(xd->base.c->c, XCB_RENDER_PICT_OP_DIFFERENCE, xd->white_pixel,
                             XCB_NONE, tmp_pict, 0, 0, 0, 0, 0, 0, tmpw, tmph);
        xcb_render_composite(xd->base.c->c, XCB_RENDER_PICT_OP_IN_REVERSE, tmp_pict2,
                             XCB_NONE, tmp_pict, 0, 0, 0, 0, 0, 0, tmpw, tmph);
    ...
    if (img->dim != 0) {
      ...
      xcb_render_fill_rectangles(xd->base.c->c, XCB_RENDER_PICT_OP_OVER, tmp_pict,
                                 dim_color, 1, &rect);
    }
    ...
    if (img->corner_radius != 0 && xrimg->rounded_rectangle != NULL) {
      ...
      xcb_render_composite(xd->base.c->c, XCB_RENDER_PICT_OP_IN_REVERSE,
                           xrimg->rounded_rectangle->p, XCB_NONE, tmp_pict, 0, 0, 0, 0, 0,
                           0, tmpw, tmph);
    }

    xcb_render_composite(xd->base.c->c, XCB_RENDER_PICT_OP_OVER, tmp_pict, mask_pict,
                         result, 0, 0, mask_dst_x, mask_dst_y, to_i16_checked(dst.x),
    ...
    xcb_render_composite(xd->base.c->c, op, inner->pict, mask_pict, result, 0, 0,
    ...
    if (img->dim != 0 || img->color_inverted) {
      ...
      if (img->color_inverted) {
        xcb_render_composite(xd->base.c->c, XCB_RENDER_PICT_OP_DIFFERENCE,
                             xd->white_pixel, XCB_NONE, result, 0, 0, 0, 0,
                             to_i16_checked(dst.x), to_i16_checked(dst.y), tmpew, tmpeh);
      }
      ...
      if (img->dim != 0) {
        ...
        xcb_render_fill_rectangles(xd->base.c->c, XCB_RENDER_PICT_OP_OVER, result,
                                   dim_color, 1, &rect);
      }
    }
  }
...
static void
compose(backend_t *base, void *img_data, coord_t dst, void *mask, coord_t mask_dst,
        const region_t *reg_paint, const region_t *reg_visible, bool lerp attr_unused) {
  struct _xrender_data *xd = (void *)base;
  return compose_impl(xd, img_data, dst, mask, mask_dst, reg_paint, reg_visible, xd->back[2]);
}
...
static bool blur(backend_t *backend_data, double opacity, void *ctx_, void *mask,
  ...
  region_t reg_op_resized = resize_region(&reg_op, bctx->resize_width, bctx->resize_height);
  ...
  xcb_render_picture_t tmp_picture[2] = {
      x_create_picture_with_visual(xd->base.c, width_resized, height_resized,
                                   xd->base.c->screen_info->root_visual, pic_attrs_mask,
                                   &pic_attrs),
      x_create_picture_with_visual(xd->base.c, width_resized, height_resized,
                                   xd->base.c->screen_info->root_visual, pic_attrs_mask,
                                   &pic_attrs)};
  ...
  if (mask != NULL) {
    mask_pict =
        process_mask(xd, mask, opacity != 1.0 ? mask_pict : XCB_NONE, &mask_allocated);
  }
  int current = 0;
  ...
  for (i = 0; i < bctx->x_blur_kernel_count; i++) {
    ...
    // window.
    xcb_render_set_picture_filter(c->c, src_pict, to_u16_checked(strlen(filter)), filter,
    ...
    if (i == 0) {
      ...
      xcb_render_composite(c->c, XCB_RENDER_PICT_OP_SRC, src_pict, XCB_NONE, dst_pict,
                           ...
                           to_i16_checked(extent_resized->y1), 0, 0, 0, 0, width_resized,
                           height_resized);
    } else if (i < bctx->x_blur_kernel_count - 1) {
      // This is not the last pass or the first pass,
      // tmp picture 1 -> tmp picture 2
      xcb_render_composite(c->c, XCB_RENDER_PICT_OP_SRC, src_pict, XCB_NONE, dst_pict, 0,
                           0, 0, 0, 0, 0, width_resized, height_resized);
    } else {
      x_set_picture_clip_region(c, xd->back[2], 0, 0, &reg_op);
      // This is the last pass, and we are doing more than 1 pass
      xcb_render_composite(c->c, XCB_RENDER_PICT_OP_OVER, src_pict, mask_pict, xd->back[2],
    ...
    // reset filter
    xcb_render_set_picture_filter(c->c, src_pict, to_u16_checked(strlen(filter0)),
                                  filter0, 0, NULL);
...
bind_pixmap(backend_t *base, xcb_pixmap_t pixmap, struct xvisual_info fmt, bool owned) {
  ...
  inner->pict = x_create_picture_with_visual_and_pixmap(base->c, fmt.visual, pixmap,
                                                        XCB_RENDER_CP_REPEAT, &pic_attrs);
  inner->owned = owned;
...
static void present(backend_t *base, const region_t *region) {
  ...
  if (xd->vsync) {
    ...
    xcb_render_composite(base->c->c, XCB_RENDER_PICT_OP_SRC, xd->back[2], XCB_NONE,
                         xd->back[xd->curr_back], orig_x, orig_y, 0, 0, orig_x, orig_y,
    ...
    auto e = xcb_request_check(
        base->c->c,
        xcb_present_pixmap_checked(xd->base.c->c, xd->target_win,
    ...
    xcb_render_composite(base->c->c, XCB_RENDER_PICT_OP_SRC, xd->back[2], XCB_NONE, xd->target,
                         orig_x, orig_y, 0, 0, orig_x, orig_y, region_width, region_height);
  }
}
...
new_inner(backend_t *base, int w, int h, xcb_visualid_t visual, uint8_t depth) {
  ...
  new_inner->pict =
      x_create_picture_with_visual_and_pixmap(base->c, visual, new_inner->pixmap, 0, NULL);
...
static void *make_mask(backend_t *base, geometry_t size, const region_t *reg) {
  ...
  auto inner = new_inner(base, size.width + 2, size.height + 2,
...
static bool image_op(backend_t *base, enum image_operations op, void *image,
  ...
  case IMAGE_OP_APPLY_ALPHA:
    ...
    xcb_render_composite(base->c->c, XCB_RENDER_PICT_OP_OUT_REVERSE, alpha_pict, XCB_NONE,
                         inner->pict, 0, 0, 0, 0, 0, 0, to_u16_checked(inner->width),
                         to_u16_checked(inner->height));
...
create_blur_context(backend_t *base attr_unused, enum blur_method method, void *args) {
  ...
  for (int i = 0; i < kernel_count; i++) {
    ...
    x_create_convolution_kernel(kernels[i], kernels[i]->data[center], &ret->x_blur_kernel[i]);
...
static backend_t *backend_xrender_init(session_t *ps, xcb_window_t target) {
  ...
  if (ps->present_exists) {
    ...
    auto e = xcb_request_check(
        ps->c.c, xcb_present_select_input_checked(ps->c.c, eid, xd->target_win,
    ...
    xd->present_event = xcb_register_for_special_xge(ps->c.c, &xcb_present_id, eid, NULL);
    if (!xd->present_event) {
  ...
  for (int i = first_buffer_index; i < 3; i++) {
    ...
    xd->back[i] = x_create_picture_with_visual_and_pixmap(
        &ps->c, ps->c.screen_info->root_visual, xd->back_pixmap[i], pic_attrs_mask, &pic_attrs);
    xd->buffer_age[i] = -1;
...
set_image_property(backend_t *base, enum image_properties op, void *image, void *args) {
  ...
  if (op == IMAGE_PROPERTY_CORNER_RADIUS && ((double *)args)[0] != xrimg->base.corner_radius) {
