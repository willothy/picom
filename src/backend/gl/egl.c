...
#undef get_proc
  ...
  gd->target_win = eglCreatePlatformWindowSurfaceProc(gd->display, config, &target, NULL);
  if (gd->target_win == EGL_NO_SURFACE) {
...
egl_bind_pixmap(backend_t *base, xcb_pixmap_t pixmap, struct xvisual_info fmt, bool owned) {
  ...
  eglpixmap->image = eglCreateImageProc(gd->display, EGL_NO_CONTEXT, EGL_NATIVE_PIXMAP_KHR,
