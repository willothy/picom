...
bool gl_kernel_blur(double opacity, struct gl_blur_context *bctx, const rect_t *extent,
  ...
  for (int i = 0; i < bctx->npasses; ++i) {
    ...
    glUseProgram(p->prog);
    glUniform2f(p->uniform_pixel_norm, 1.0F / (GLfloat)tex_width, 1.0F / (GLfloat)tex_height);
    ...
    if (i < bctx->npasses - 1) {
      ...
      if (mask) {
        ...
        glUniform1f(p->uniform_mask_corner_radius, (float)mask->corner_radius);
        glUniform2f(p->uniform_mask_offset, (float)(mask_dst.x),
  ...
  for (int i = iterations - 1; i >= 0; --i) {
    ...
    if (i > 0) {
      ...
      if (mask) {
        ...
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, inner->texture);
        glUniform1i(up_pass->uniform_mask_inverted, mask->color_inverted);
        glUniform1f(up_pass->uniform_mask_corner_radius, (float)mask->corner_radius);
        glUniform2f(up_pass->uniform_mask_offset, (float)(mask_dst.x),
                    (float)(bctx->fb_height - mask_dst.y - inner->height));
      }
      glBindVertexArray(vao[0]);
      nelems = vao_nelems[0];
...
bool gl_blur_impl(double opacity, struct gl_blur_context *bctx, void *mask, coord_t mask_dst,
  ...
  if (source_size.width != bctx->fb_width || source_size.height != bctx->fb_height) {
    ...
    for (int i = 0; i < bctx->blur_texture_count; ++i) {
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, format, tex_size->width, tex_size->height, 0,
  ...
  auto reg_blur_resized = resize_region(reg_blur, bctx->resize_width, bctx->resize_height);
  ...
  x_rect_to_coords(nrects, rects, (coord_t){.x = extent_resized->x1, .y = extent_resized->y1},
  ...
  x_rect_to_coords(nrects_resized, rects_resized,
                   (coord_t){.x = extent_resized->x1, .y = extent_resized->y1}, extent_height,
                   bctx->fb_height, bctx->fb_height, false, coord_resized, indices_resized);
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, (long)sizeof(*indices) * nrects * 6, indices,
               GL_STREAM_DRAW);
  glEnableVertexAttribArray(vert_coord_loc);
  glEnableVertexAttribArray(vert_in_texcoord_loc);
  glVertexAttribPointer(vert_coord_loc, 2, GL_INT, GL_FALSE, sizeof(GLint) * 4, NULL);
  glVertexAttribPointer(vert_in_texcoord_loc, 2, GL_INT, GL_FALSE, sizeof(GLint) * 4,
                        (void *)(sizeof(GLint) * 2));
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, (long)sizeof(*indices_resized) * nrects_resized * 6,
               indices_resized, GL_STREAM_DRAW);
  glEnableVertexAttribArray(vert_coord_loc);
  glEnableVertexAttribArray(vert_in_texcoord_loc);
  glVertexAttribPointer(vert_coord_loc, 2, GL_INT, GL_FALSE, sizeof(GLint) * 4, NULL);
  glVertexAttribPointer(vert_in_texcoord_loc, 2, GL_INT, GL_FALSE, sizeof(GLint) * 4,
  ...
  if (bctx->method == BLUR_METHOD_DUAL_KAWASE) {
    ret = gl_dual_kawase_blur(opacity, bctx, extent_resized, mask, mask_dst, vao, vao_nelems,
                              source_texture, source_size, target_fbo, default_mask);
  } else {
    ret = gl_kernel_blur(opacity, bctx, extent_resized, mask, mask_dst, vao, vao_nelems,
                         source_texture, source_size, target_fbo, default_mask);
  }
...
bool gl_blur(backend_t *base, double opacity, void *ctx, void *mask, coord_t mask_dst,
  ...
  return gl_blur_impl(opacity, bctx, mask, mask_dst, reg_blur, reg_visible, gd->back_texture,
...
bool gl_create_kernel_blur_context(void *blur_context, GLfloat *projection,
  ...
  for (int i = 0; i < nkernels; i++) {
    ...
    if (width > height) {
      ...
      for (int j = 0; j < height; ++j) {
        for (int k = 0; k < width; k += 2) {
          ...
          val2 = (k + 1 < width) ? kern->data[j * width + k + 1] : 0;
          ...
          double offset_x = k + (val2 / combined_weight) - (width / 2);
          double offset_y = j - (height / 2);
          pc += snprintf(pc, body_len - (ulong)(pc - shader_body), shader_add,
                         combined_weight, offset_x, offset_y);
      ...
      for (int j = 0; j < height; j += 2) {
        for (int k = 0; k < width; ++k) {
          double val1, val2;
          val1 = kern->data[j * width + k];
          val2 = (j + 1 < height) ? kern->data[(j + 1) * width + k] : 0;
          ...
          double offset_y = j + (val2 / combined_weight) - (height / 2);
          pc += snprintf(pc, body_len - (ulong)(pc - shader_body), shader_add,
                         combined_weight, offset_x, offset_y);
          assert(pc < shader_body + body_len);
        }
    ...
    size_t shader_len = strlen(FRAG_SHADER_BLUR) + strlen(extension) +
                        strlen(shader_body) + 10 /* sum */ + 1 /* null terminator */;
    char *shader_str = ccalloc(shader_len, char);
    auto real_shader_len =
        snprintf(shader_str, shader_len, FRAG_SHADER_BLUR, extension, shader_body, sum);
    ...
    // Build program
    pass->prog =
        gl_create_program_from_strv((const char *[]){vertex_shader, NULL},
...
bool gl_create_dual_kawase_blur_context(void *blur_context, GLfloat *projection,
  ...
  auto down_pass = ctx->blur_shader;
  {
    ...
    down_pass->texorig_loc = glGetUniformLocationChecked(down_pass->prog, "texorig");
    down_pass->scale_loc = glGetUniformLocationChecked(down_pass->prog, "scale");
  ...
  auto up_pass = ctx->blur_shader + 1;
  {
    ...
    // Build shader
    size_t shader_len = strlen(FRAG_SHADER_UP) + 10 /* offset */ + 1 /* null terminator */;
    char *shader_str = ccalloc(shader_len, char);
    auto real_shader_len =
    ...
    up_pass->texorig_loc = glGetUniformLocationChecked(up_pass->prog, "texorig");
...
void *gl_create_blur_context(backend_t *base, enum blur_method method, void *args) {
  ...
  if (method == BLUR_METHOD_DUAL_KAWASE) {
    success = gl_create_dual_kawase_blur_context(ctx, projection_matrix[0], method, args);
  } else {
    success = gl_create_kernel_blur_context(ctx, projection_matrix[0], method, args);
  }
