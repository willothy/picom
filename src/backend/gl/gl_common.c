...
GLuint gl_create_program_from_strv(const char **vert_shaders, const char **frag_shaders) {
  ...
  for (int i = 0; i < frag_count; ++i) {
    shaders[vert_count + i] = gl_create_shader(GL_FRAGMENT_SHADER, frag_shaders[i]);
...
_gl_average_texture_color(backend_t *base, GLuint source_texture, GLuint destination_texture,
  ...
  if (to_width > max_width || to_height > max_height) {
    ...
    result = _gl_average_texture_color(base, new_source_texture, new_destination_texture,
                                       0, fbo, to_width, to_height);
  } else {
...
static GLuint gl_average_texture_color(backend_t *base, struct backend_image *img) {
  ...
  if (!inner->auxiliary_texture[0]) {
    ...
    for (int i = 0; i < texture_count; i++) {
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, inner->width, inner->height, 0, GL_BGR,
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, (long)sizeof(*indices) * 6, indices, GL_STREAM_DRAW);

  // Do actual recursive render to 1x1 texture
  GLuint result_texture = _gl_average_texture_color(
      base, inner->texture, inner->auxiliary_texture[0], inner->auxiliary_texture[1], fbo,
...
static void _gl_compose(backend_t *base, struct backend_image *img, GLuint target,
  ...
  if (win_shader->uniform_effective_size >= 0) {
    glUniform2f(win_shader->uniform_effective_size, (float)img->ewidth, (float)img->eheight);
  }
  ...
  glUniform2f(win_shader->uniform_mask_offset, (float)mask_offset.x, (float)mask_offset.y);
  if (mask != NULL) {
    glUniform1i(win_shader->uniform_mask_inverted, mask->color_inverted);
    glUniform1f(win_shader->uniform_mask_corner_radius, (GLfloat)mask->corner_radius);
  } else {
...
void x_rect_to_coords(int nrects, const rect_t *rects, coord_t image_dst,
  ...
  for (int i = 0; i < nrects; i++) {
    ...
    GLint texture_x1 = crect.x1 - image_dst.x, texture_y1 = crect.y2 - image_dst.y,
...
void gl_compose(backend_t *base, void *image_data, coord_t image_dst, void *mask,
  ...
  x_rect_to_coords(nrects, rects, image_dst, inner->height, inner->height, gd->height,
...
static void _gl_fill(backend_t *base, struct color c, const region_t *clip, GLuint target,
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, nrects * 6 * (long)sizeof(*indices), indices,
               GL_STREAM_DRAW);

  glVertexAttribPointer(fill_vert_in_coord_loc, 2, GL_INT, GL_FALSE, sizeof(*coord) * 2,
                        (void *)0);
  glBindFramebuffer(GL_DRAW_FRAMEBUFFER, target);
  glDrawElements(GL_TRIANGLES, nrects * 6, GL_UNSIGNED_INT, NULL);
...
bool gl_init(struct gl_data *gd, session_t *ps) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, 1, 1, 0, GL_RED, GL_UNSIGNED_BYTE, (GLbyte[]){'\xff'});
  glBindTexture(GL_TEXTURE_2D, 0);
...
static inline void gl_image_decouple(backend_t *base, struct backend_image *img) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, new_tex->width, new_tex->height, 0, GL_BGRA,
               GL_UNSIGNED_BYTE, NULL);
  glBindTexture(GL_TEXTURE_2D, 0);
...
bool gl_last_render_time(backend_t *base, struct timespec *ts) {
  ...
  GLuint64 time;
  glGetQueryObjectui64v(gd->frame_timing[gd->current_frame_timing ^ 1], GL_QUERY_RESULT, &time);
...
void *gl_shadow_from_mask(backend_t *base, void *mask,
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, new_inner->width, new_inner->height, 0, GL_RED,
               GL_UNSIGNED_BYTE, NULL);
  glBindTexture(GL_TEXTURE_2D, 0);
  GLuint fbo;
  glGenFramebuffers(1, &fbo);
  ...
  if (gsctx->blur_context != NULL) {
    ...
    // that.
    gl_blur_impl(1.0, gsctx->blur_context, NULL, (coord_t){0}, &reg_blur, NULL, source_texture,
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, new_inner->width, new_inner->height, 0, GL_RGBA,
               GL_UNSIGNED_BYTE, NULL);
  glBindFramebuffer(GL_DRAW_FRAMEBUFFER, fbo);
  glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                         new_inner->texture, 0);
  ...
  glUniform4f(gd->shadow_shader.uniform_color, (GLfloat)(color.red * color.alpha),
              (GLfloat)(color.green * color.alpha), (GLfloat)(color.blue * color.alpha),
              (GLfloat)color.alpha);
