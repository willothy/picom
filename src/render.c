...
static inline bool paint_bind_tex(session_t *ps, paint_t *ppaint, int wid, int hei,
  ...
  if (!visual) {
    ...
    if (!ps->argb_fbconfig) {
      ps->argb_fbconfig =
          glx_find_fbconfig(&ps->c, (struct xvisual_info){.red_size = 8,
                                                          .green_size = 8,
                                                          .blue_size = 8,
                                                          .alpha_size = 8,
                                                          .visual_depth = 32});
  ...
  if (force || !glx_tex_bound(ppaint->ptex, ppaint->pixmap)) {
    return glx_bind_pixmap(ps, &ppaint->ptex, ppaint->pixmap, wid, hei, repeat, fbcfg);
  }
#else
  (void)ps;
  (void)ppaint;
...
static int get_buffer_age(session_t *ps) {
  ...
  if (bkend_use_glx(ps)) {
    ...
    if (ps->o.use_damage) {
      unsigned int val;
      glXQueryDrawable(ps->c.dpy, get_tgt_window(ps), GLX_BACK_BUFFER_AGE_EXT, &val);
      return (int)val ?: -1;
    }
    return -1;
  }
    ...
    if (alpha_step != 0) {
      if (cr) {
        ...
        const xcb_rectangle_t rect = {
            .x = 0, .y = 0, .width = to_u16_checked(fullwid), .height = to_u16_checked(fullhei)};
        xcb_render_fill_rectangles(ps->c.c, XCB_RENDER_PICT_OP_SRC, p_tmp, trans, 1, &rect);
        ...
        xcb_render_trapezoids(ps->c.c, XCB_RENDER_PICT_OP_OVER, alpha_pict, p_tmp,
                              x_get_pictfmt_for_standard(&ps->c, XCB_PICT_STANDARD_A_8),
                              0, 0, n, traps);

        xcb_render_composite(ps->c.c, XCB_RENDER_PICT_OP_OVER, pict, p_tmp,
                             ps->tgt_buffer.pict, to_i16_checked(x), to_i16_checked(y),
                             to_i16_checked(x), to_i16_checked(y), to_i16_checked(dx),
        ...
        if (clip) {
          p_tmp = x_create_picture_with_standard(&ps->c, wid, hei,
                                                 XCB_PICT_STANDARD_ARGB_32, 0, 0);

          xcb_render_color_t black = {.red = 255, .blue = 255, .green = 255, .alpha = 255};
          const xcb_rectangle_t rect = {
              .x = 0, .y = 0, .width = to_u16_checked(wid), .height = to_u16_checked(hei)};
          xcb_render_fill_rectangles(ps->c.c, XCB_RENDER_PICT_OP_SRC, p_tmp, black, 1, &rect);
          if (alpha_pict) {
            xcb_render_composite(ps->c.c, XCB_RENDER_PICT_OP_SRC, alpha_pict, XCB_NONE, p_tmp,
                                 0, 0, 0, 0, 0, 0, to_u16_checked(wid), to_u16_checked(hei));
          }
          xcb_render_composite(ps->c.c, XCB_RENDER_PICT_OP_OUT_REVERSE, clip->pict,
                               XCB_NONE, p_tmp, 0, 0, 0, 0, to_i16_checked(clip->x),
                               to_i16_checked(clip->y), to_u16_checked(wid),
                               to_u16_checked(hei));
        }
        uint8_t op = ((!argb && !alpha_pict && !clip) ? XCB_RENDER_PICT_OP_SRC
                                                      : XCB_RENDER_PICT_OP_OVER);

        xcb_render_composite(ps->c.c, op, pict, p_tmp, ps->tgt_buffer.pict,
...
#ifdef CONFIG_OPENGL
  case BKEND_GLX:
    glx_render(ps, ptex, x, y, dx, dy, wid, hei, ps->psglx->z, opacity, argb, neg,
               reg_paint, pprogram);
    ps->psglx->z += 1;
    break;
#endif
...
paint_region(session_t *ps, const struct managed_win *w, int x, int y, int wid, int hei,
  ...
  render(ps, x, y, dx, dy, wid, hei, fullwid, fullhei, opacity, argb, neg,
         w ? w->corner_radius : 0, pict, (w ? w->paint.ptex : ps->root_tile_paint.ptex),
         reg_paint,
#ifdef CONFIG_OPENGL
...
void paint_one(session_t *ps, struct managed_win *w, const region_t *reg_paint) {
  ...
  if (!w->paint.pixmap) {
    ...
    set_ignore_cookie(
        &ps->c, xcb_composite_name_window_pixmap(ps->c.c, w->base.id, w->paint.pixmap));
  }
  ...
  if (bkend_use_xrender(ps) && w->invert_color) {
    ...
    if (newpict) {
      ...
      xcb_render_composite(ps->c.c, XCB_RENDER_PICT_OP_SRC, pict, XCB_NONE, newpict, 0, 0,
                           0, 0, 0, 0, wid, hei);
      xcb_render_composite(ps->c.c, XCB_RENDER_PICT_OP_DIFFERENCE, ps->white_picture,
      ...
      if (win_has_alpha(w)) {
        xcb_render_composite(ps->c.c, XCB_RENDER_PICT_OP_IN_REVERSE, pict, XCB_NONE,
...
#define COMP_BDR(cx, cy, cwid, chei)                                                     \
  paint_region(ps, w, (cx), (cy), (cwid), (chei), w->frame_opacity * w->opacity,         \
    ...
    do {
      ...
      // body
      paint_region(ps, w, cleft, ctop, body_width, body_height, w->opacity, reg_paint, pict);
    } while (0);
  }
...
#undef COMP_BDR
  ...
  if (w->dim) {
    ...
    case BKEND_XR_GLX_HYBRID: {
      ...
      xcb_render_fill_rectangles(ps->c.c, XCB_RENDER_PICT_OP_OVER, ps->tgt_buffer.pict,
                                 color, 1, &rect);
    } break;
#ifdef CONFIG_OPENGL
    case BKEND_GLX:
      glx_dim_dst(ps, x, y, wid, hei, (int)(ps->psglx->z - 0.7), (float)dim_opacity, reg_paint);
      break;
#endif
    default: assert(false);
    }
...
static bool get_root_tile(session_t *ps) {
  ...
  if (!pixmap || !r) {
    pixmap = x_create_pixmap(&ps->c, (uint8_t)ps->c.screen_info->root_depth, 1, 1);
    if (pixmap == XCB_NONE) {
  ...
  if (fill) {
    ...
    xcb_render_fill_rectangles(ps->c.c, XCB_RENDER_PICT_OP_SRC, ps->root_tile_paint.pict,
                               col, 1, &rect);
  }
...
static bool win_build_shadow(session_t *ps, struct managed_win *w, double opacity) {
  ...
  shadow_pixmap_argb = x_create_pixmap(&ps->c, 32, shadow_image->width, shadow_image->height);
...
win_paint_shadow(session_t *ps, struct managed_win *w, region_t *reg_paint) {
  ...
  if (should_clip) {
    if (ps->o.backend == BKEND_XRENDER || ps->o.backend == BKEND_XR_GLX_HYBRID) {
      ...
      xcb_render_trapezoid_t traps[4 * max_ntraps + 3];
      uint32_t n = make_rounded_window_shape(traps, max_ntraps, w->corner_radius,
                                             w->widthb, w->heightb);

      td = x_create_picture_with_standard(&ps->c, w->widthb, w->heightb,
                                          XCB_PICT_STANDARD_ARGB_32, 0, 0);
      xcb_render_color_t trans = {.red = 0, .blue = 0, .green = 0, .alpha = 0};
      ...
      xcb_render_fill_rectangles(ps->c.c, XCB_RENDER_PICT_OP_SRC, td, trans, 1, &rect);

      auto solid = solid_picture(&ps->c, false, 1, 0, 0, 0);
      xcb_render_trapezoids(ps->c.c, XCB_RENDER_PICT_OP_OVER, solid, td,
...
xr_blur_dst(session_t *ps, xcb_render_picture_t tgt_buffer, int16_t x, int16_t y,
  ...
  for (int i = 0; i < nkernels; ++i) {
    ...
    // window.
    xcb_render_set_picture_filter(ps->c.c, src_pict, strlen(XRFILTER_CONVOLUTION),
                                  XRFILTER_CONVOLUTION, (uint32_t)(kwid * khei + 2),
                                  convolution_blur);
    xcb_render_composite(ps->c.c, XCB_RENDER_PICT_OP_SRC, src_pict, XCB_NONE, dst_pict,
  ...
  if (src_pict != tgt_buffer) {
    xcb_render_composite(ps->c.c, XCB_RENDER_PICT_OP_OVER, src_pict, rounded, tgt_buffer,
                         0, 0, 0, 0, x, y, wid, hei);
  }
...
win_blur_background(session_t *ps, struct managed_win *w, xcb_render_picture_t tgt_buffer,
  ...
  case BKEND_XR_GLX_HYBRID: {
    ...
    for (int i = 0; i < ps->o.blur_kernel_count; i++) {
      ...
      x_create_convolution_kernel(kern_src, factor_center, &ps->blur_kerns_cache[i]);
    }
    ...
    if (cr) {
      ...
      td = x_create_picture_with_standard(&ps->c, wid, hei, XCB_PICT_STANDARD_ARGB_32, 0, 0);
      xcb_render_color_t trans = {.red = 0, .blue = 0, .green = 0, .alpha = 0};
      const xcb_rectangle_t rect = {
          .x = 0, .y = 0, .width = to_u16_checked(wid), .height = to_u16_checked(hei)};
      xcb_render_fill_rectangles(ps->c.c, XCB_RENDER_PICT_OP_SRC, td, trans, 1, &rect);
  ...
  case BKEND_GLX:
    ...
    glx_blur_dst(ps, x, y, wid, hei, (float)ps->psglx->z - 0.5F, (float)factor_center,
...
void paint_all(session_t *ps, struct managed_win *t) {
  ...
  if (!paint_isvalid(ps, &ps->tgt_buffer)) {
    if (!ps->tgt_buffer.pixmap) {
      ...
      ps->tgt_buffer.pixmap = x_create_pixmap(&ps->c, ps->c.screen_info->root_depth,
    ...
    if (BKEND_GLX != ps->o.backend) {
      ps->tgt_buffer.pict = x_create_picture_with_visual_and_pixmap(
          &ps->c, ps->c.screen_info->root_visual, ps->tgt_buffer.pixmap, 0, 0);
    }
  }
  ...
  for (auto w = t; w; w = w->prev_trans) {
    region_t bshape_no_corners = win_get_bounding_shape_global_without_corners_by_val(w);
    ...
    if (w->shadow) {
      ...
      if (ps->o.crop_shadow_to_monitor && w->randr_monitor >= 0 &&
        ...
        // bounds.
        pixman_region32_intersect(&reg_tmp, &reg_tmp, &ps->monitors.regions[w->randr_monitor]);
      }
    ...
    if (w->opacity * MAX_ALPHA >= 1) {
      if (w->clip_shadow_above) {
        ...
        pixman_region32_union(&reg_shadow_clip, &reg_shadow_clip, &bshape_corners);
      } else {
        // Remove overlapping window bounds from shadow-clip
        // region
        pixman_region32_subtract(&reg_shadow_clip, &reg_shadow_clip, &bshape_corners);
      }
    }
    ...
    if (pixman_region32_not_empty(&reg_tmp)) {
      ...
      if (w->blur_background && (w->mode == WMODE_TRANS ||
      ...
      if (w->corner_radius > 0 && ps->o.backend == BKEND_GLX) {
        auto const wid = to_u16_checked(w->widthb);
        auto const hei = to_u16_checked(w->heightb);
        glx_round_corners_dst(ps, w, w->glx_texture_bg, w->g.x, w->g.y, wid, hei,
                              (float)ps->psglx->z - 0.5F, (float)w->corner_radius, &reg_tmp);
      }
#endif
    }
  }
  ...
  case BKEND_XRENDER:
    if (ps->o.monitor_repaint) {
      ...
      xcb_render_picture_t new_pict =
          x_create_picture_with_pictfmt(&ps->c, rwidth, rheight, pictfmt, 0, NULL);
      xcb_render_composite(ps->c.c, XCB_RENDER_PICT_OP_SRC, ps->tgt_buffer.pict, XCB_NONE,
      ...
      xcb_render_composite(ps->c.c, XCB_RENDER_PICT_OP_SRC, new_pict, XCB_NONE,
                           ps->tgt_picture, 0, 0, 0, 0, 0, 0, rwidth, rheight);
  ...
  case BKEND_XR_GLX_HYBRID:
    ...
    paint_bind_tex(ps, &ps->tgt_buffer, ps->root_width, ps->root_height, false,
                   ps->c.screen_info->root_depth, ps->c.screen_info->root_visual,
    ...
    glXWaitX();
    glx_render(ps, ps->tgt_buffer.ptex, 0, 0, 0, 0, ps->root_width, ps->root_height, 0,
               1.0, false, false, &region, NULL);
    fallthrough();
...
bool init_render(session_t *ps) {
  ...
  if (ps->o.blur_method == BLUR_METHOD_KERNEL) {
    ps->blur_kerns_cache = ccalloc(ps->o.blur_kernel_count, struct x_convolution_kernel *);
